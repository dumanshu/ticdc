...
func TestGetCredential(t *testing.T) {
	o := newOptions()
	o.caPath = "/ca/path"
	o.certPath = "/cert/path"
	o.keyPath = "/key/path"
	o.allowedCertCN = "test-cn"

	cred := o.getCredential()

	require.Equal(t, "/ca/path", cred.CAPath)
	require.Equal(t, "/cert/path", cred.CertPath)
	require.Equal(t, "/key/path", cred.KeyPath)
	require.Equal(t, []string{"test-cn"}, cred.CertAllowedCN)
}

func TestGetCredentialMultipleCN(t *testing.T) {
	o := newOptions()
	o.allowedCertCN = "cn1,cn2,cn3"

	cred := o.getCredential()

	require.Equal(t, []string{"cn1", "cn2", "cn3"}, cred.CertAllowedCN)
}

func TestGetCredentialEmptyCN(t *testing.T) {
	o := newOptions()
	o.allowedCertCN = ""

	cred := o.getCredential()

	require.Nil(t, cred.CertAllowedCN)
}
